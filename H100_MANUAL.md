# H100 run manual — Business Entity Resolution (V2 → V3 → phase-2 diagnosis → V4)
...
```bash
bash run_all.sh --data /path/to/dataset            # everything: v2 → v3 → audit → phase2 → loco → select → v4
...
| disk full | `data/cache_v2/p2/` and `data/cache_v2/emb/e3_test/paircos5_c2.parquet` can be deleted after phase 2 |
